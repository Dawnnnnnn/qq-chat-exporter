...
(function () {
  ...
  async function initSilkAudioPlayer() {
    ...
    log('Found ' + silkAudios.length + ' Silk audio files, initializing decoder...');
    ...
    try {
      ...
      for (let i = 0; i < silkAudios.length; i++) {
        ...
        try {
          ...
          warn('Failed to decode audio:', originalSrc, err);
          const wrapper = audioEl.closest('.audio-wrapper');
          if (wrapper) {
      ...
      log('Silk audio decoding completed');
    } catch (error) {
      warn('Failed to initialize Silk decoder:', error);
    }
  }
  ...
  function init() {
    ...
    // initialize silk audio player (async, non-blocking)
    initSilkAudioPlayer().catch(function(e) {
      warn('Silk audio init failed:', e);
    });
